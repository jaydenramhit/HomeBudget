...
namespace BudgetCodeTests
{
    ...
    public class TestCategories
    {
        ...
        [Fact]
        public void CategoriesMethod_Delete_ForeignKeyConstraint_RestrictsDelete()
        {
            // Arrange
            String folder = TestConstants.GetSolutionDir();
            String goodDB = $"{folder}\\{TestConstants.testDBInputFile}";
            ...
            // Act
            // Deleting a foreign key reference should throw an exception and leave the table unchanged
            ...
            // Get the category to make sure it wasn't changed
            Category category = categories.GetCategoryFromId(catID);

            // Deleting a non foreign key reference shouldn't cause any issues
            try
            {
                categories.Delete(noIssueCatID);
            }
            catch (System.Data.SQLite.SQLiteException)
            {
                Assert.False(true);
            ...
            Assert.Equal("Clothes", category.Description);
